...
//        runDemo(netPresenter, gson);
        ...
        //3.2.35.	查询上映该电影的影院
        netPresenter.getMovieOnCinema(tuid, uuid, "北京", "218090", lat, lon, new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                LogUtil.i(TAG, "getMovieOnCinema: failure");
            }

            @Override
            public void onResponse(Call call, Response response) throws IOException {
                LogUtil.i(TAG, "getMovieOnCinema: response.body: " + response.body().string());
            }
        });
...
//                LogUtil.i(TAG, "getWeatherInfo: " + bean.getResp_data().getCity_name());
        ...
        netPresenter.getDelicacySetting(tuid, uuid, "1", new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                LogUtil.i(TAG, "getDelicacySetting: setting list size: " + settingBean.getSetting_list().size() + ", response.body: " + data);
            }
        });
        ...
        // 3.2.27.	获取周边充电站信息
        String str = "";
        netPresenter.getChargingStation(tuid, uuid, lat, lon, "1", "10", str, str,
                str, str, new Callback() {
                    @Override
                    public void onFailure(Call call, IOException e) {
        ...
        // 3.2.32.	获取周边电影院查询信息
        netPresenter.getMovieCinema(tuid, uuid, lat, lon, "3000", new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                LogUtil.i(TAG, "getMovieCinema: failure");
            }

            @Override
            public void onResponse(Call call, Response response) throws IOException {
                LogUtil.i(TAG, "getMovieCinema: response.body: " + response.body().string());
            }
        });

        // 3.2.33.	电影院热映电影查询及排期信息
        netPresenter.getMovieScheduleInCinema(tuid, uuid, "152", "218090", new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                LogUtil.i(TAG, "getMovieScheduleInCinema: failure");
            }

            @Override
            public void onResponse(Call call, Response response) throws IOException {
                LogUtil.i(TAG, "getMovieScheduleInCinema: response.body: " + response.body().string());
            }
        });

        // 3.2.34.	今天放映影片查询
        netPresenter.getMovieHotToday(tuid, uuid, "北京", new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                LogUtil.i(TAG, "getMovieHotToday: failure");
            }

            @Override
            public void onResponse(Call call, Response response) throws IOException {
                LogUtil.i(TAG, "getMovieHotToday: response.body: " + response.body().string());
            }
        });
        ...
        // 3.2.42.	根据影片ID查询影片信息
        netPresenter.getMovieInfo(tuid, uuid, "218090", new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                LogUtil.i(TAG, "getMovieInfo: failure");
            }

            @Override
            public void onResponse(Call call, Response response) throws IOException {
                LogUtil.i(TAG, "getMovieInfo: response.body: " + response.body().string());
            }
        });

        // 3.2.43.	获取智慧导航功能项列表
        netPresenter.getSmartNavi(tuid, uuid, new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                LogUtil.i(TAG, "getSmartNavi: failure");
            }

            @Override
            public void onResponse(Call call, Response response) throws IOException {
                LogUtil.i(TAG, "getSmartNavi: response.body: " + response.body().string());
            }
        });
    }
}
